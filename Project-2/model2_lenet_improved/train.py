"""
Model 2 — LeNet-5 (iyileştirilmiş) eğitim scripti

Veri seti : CIFAR-10
Mimari    : LeNet5Improved (Conv + BN + ReLU + MaxPool + FC + BN + Dropout)
Loss      : CrossEntropyLoss
Optimizer : Adam (lr=1e-3)      ← Model 1 ile AYNI
Batch     : 64                  ← Model 1 ile AYNI
Epoch     : 20                  ← Model 1 ile AYNI
Seed      : 42                  ← Model 1 ile AYNI
"""
...
from model import LeNet5Improved

# ───────────────────────── Hiperparametreler (Model 1 ile birebir aynı) ─────────────────────────
SEED = 42
BATCH_SIZE = 64
LR = 1e-3
EPOCHS = 20
...
def plot_curve(history, ycols, ylabel, path):
    ...
    plt.xlabel("Epoch")
    plt.ylabel(ylabel)
    plt.title(f"Model 2 (LeNet-5 İyileştirilmiş) — {ylabel}")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
...
def plot_confusion_matrix(cm, path):
    ...
    plt.xlabel("Tahmin")
    plt.ylabel("Gerçek")
    plt.title("Model 2 — Karmaşıklık Matrisi (Test)")
    plt.tight_layout()
    plt.savefig(path, dpi=150)
    plt.close()
...
def main():
    ...
    model = LeNet5Improved(num_classes=NUM_CLASSES).to(DEVICE)
    ...
    metrics = {
        "model": "Model 2 — LeNet-5 İyileştirilmiş (BN + Dropout)",
        "dataset": "CIFAR-10",
        "device": str(DEVICE),
        "epochs": EPOCHS,
